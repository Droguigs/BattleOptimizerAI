    ...
    def teampreview(self, battle):
        ...
        for i, mon in enumerate(battle.team.values()):
            ...
            mon_performance[i] = np.mean(
                [
                    teampreview_performance(mon, opp, self)
...
def teampreview_performance(mon_a, mon_b, caller):
    # We evaluate the performance on mon_a against mon_b as its type advantage
    a_on_b = b_on_a = -np.inf
    type_chart = data.GenData.load_type_chart(caller, gen=8)
    for type_ in mon_a.types:
        if type_:
            a_on_b = max(
                a_on_b, type_.damage_multiplier(
                        *mon_b.types,
    ...
    for type_ in mon_b.types:
        if type_:
            b_on_a = max(
                b_on_a, type_.damage_multiplier(
                    *mon_a.types,
                    type_chart = type_chart)
                )
